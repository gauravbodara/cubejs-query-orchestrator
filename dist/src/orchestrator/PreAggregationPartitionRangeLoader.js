...
exports.PreAggregationPartitionRangeLoader = void 0;
...
class PreAggregationPartitionRangeLoader {
    ...
    async loadRangeQuery(rangeQuery, partitionRange) {
        ...
        const invalidate = this.preAggregation.invalidateKeyQueries?.[0]
    ...
    replacePartitionSqlAndParams(query, dateRange, partitionTableName) {
        ...
        return [sql.replace(this.preAggregation.tableName, partitionTableName), params?.map(param => {
                if (dateRange && param === shared_1.FROM_PARTITION_RANGE) {
                    ...
                    return (0, shared_1.localTimestampToUtc)(this.preAggregation.timezone, this.preAggregation.timestampFormat, dateRange[0]);
                }
                else if (dateRange && param === shared_1.TO_PARTITION_RANGE) {
                    return (0, shared_1.localTimestampToUtc)(this.preAggregation.timezone, this.preAggregation.timestampFormat, dateRange[1]);
                }
                else {
                    return param;
                }
    ...
    async loadPreAggregations() {
        if (this.preAggregation.partitionGranularity && !this.preAggregation.expandedPartition) {
            const loadPreAggregationsByPartitionRanges = async ({ buildRange, partitionRanges }) => {
                const partitionLoaders = partitionRanges.map(range => new PreAggregationLoader_1.PreAggregationLoader(this.driverFactory, this.logger, this.queryCache, this.preAggregations, this.partitionPreAggregationDescription(range, buildRange), this.preAggregationsTablesToTempTables, this.loadCache, this.options));
            ...
            if (this.options.externalRefresh && loadResultAndLoaders.loadResults.length === 0) {
                ...
                PreAggregations_1.PreAggregations.noPreAggregationPartitionsBuiltMessage(loadResultAndLoaders.partitionLoaders.map(p => p.preAggregation)));
            }
            ...
            let lastUpdatedAt = (0, PreAggregations_1.getLastUpdatedAtTimestamp)(loadResults.map(r => r.lastUpdatedAt));
            if (lambdaTable) {
            ...
            return new PreAggregationLoader_1.PreAggregationLoader(this.driverFactory, this.logger, this.queryCache, this.preAggregations, this.preAggregation, this.preAggregationsTablesToTempTables, this.loadCache, this.options).loadPreAggregation(true);
        }
    }
    /**
    ...
    async downloadLambdaTable(fromDate, lambdaTypes) {
        ...
        return {
            name: `${PreAggregations_1.LAMBDA_TABLE_PREFIX}_${this.preAggregation.tableName.replace('.', '_')}`,
            columns: data.types,
    ...
    async loadBuildRange(timestampFormat = DEFAULT_TS_FORMAT) {
        const { preAggregationStartEndQueries } = this.preAggregation;
        const [startDate, endDate] = await Promise.all(preAggregationStartEndQueries.map(async (rangeQuery) => PreAggregationPartitionRangeLoader.extractDate(await this.loadRangeQuery(rangeQuery), this.preAggregation.timezone, timestampFormat)));
        ...
        const [rangeStart, rangeEnd] = await Promise.all(preAggregationStartEndQueries.map(async (rangeQuery, i) => PreAggregationPartitionRangeLoader.extractDate(await this.loadRangeQuery(rangeQuery, i === 0 ? wholeSeriesRanges[0] : wholeSeriesRanges[wholeSeriesRanges.length - 1]), this.preAggregation.timezone, timestampFormat)));
        return this.orNowIfEmpty([rangeStart, rangeEnd]);
    }
    now() {
        return (0, shared_1.utcToLocalTimeZone)(this.preAggregation.timezone, DEFAULT_TS_FORMAT, new Date().toJSON().substring(0, 23));
    }
    ...
    static partitionTableName(tableName, partitionGranularity, dateRange) {
        ...
        const partitionSuffix = dateRange[0].substring(0, dateLenCut).replace(/[-T:]/g, '');
        return `${tableName}${partitionSuffix}`;
    }
    static inDbTimeZone(preAggregationDescription, timestamp) {
        return (0, shared_1.localTimestampToUtc)(preAggregationDescription.timezone, preAggregationDescription.timestampFormat, timestamp);
    }
    static extractDate(data, timezone, timestampFormat = DEFAULT_TS_FORMAT) {
        return (0, shared_1.parseLocalDate)(data, timezone, timestampFormat);
    }
...
//# sourceMappingURL=PreAggregationPartitionRangeLoader.js.map
