...
class QueryOrchestrator {
    ...
    constructor(redisPrefix, driverFactory, logger, options = {}) {
        ...
        if (!['memory', 'cubestore'].includes(cacheAndQueueDriver)) {
            throw new Error(`Only 'cubestore' or 'memory' are supported for cacheAndQueueDriver option, passed: ${cacheAndQueueDriver}`);
        }
    ...
    getQueueEventsBus() {
        if (!this.queueEventsBus) {
            this.queueEventsBus = new LocalQueueEventsBus_1.LocalQueueEventsBus();
        }
